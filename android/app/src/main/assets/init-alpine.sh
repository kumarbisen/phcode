...
Welcome to Alpine Linux in PhCode!
...
if [ "$FAILSAFE" != true ]; then
    ...
    if command -v bash >/dev/null 2>&1; then
        if ! command -v script >/dev/null 2>&1; then
            echo -e "\e[33m[*] Installing PTY support (util-linux)...\e[0m"
            apk add --no-cache util-linux >/dev/null 2>&1
        fi
        
        sleep 0.5
        # Clear the screen completely and reset cursor to top-left
        echo -e "\033[2J\033[H\c"
        echo -e "\r\n\e[1;32m✅ PhCode Environment Booted Successfully!\e[0m\r\n"
        
        exec script -q -c "bash --rcfile /initrc -i" /dev/null
    else
        echo "bash is not installed; starting /bin/sh. Run: apk add bash util-linux"
        sleep 0.5
        echo -e "\033[2J\033[H\c"
        echo -e "\r\n\e[1;32m✅ PhCode Environment Booted Successfully!\e[0m\r\n"
        exec sh -i
