...
echo -e "\e[36m[*] Initializing PRoot Sandbox Environment...\e[0m"
sleep 0.4
mkdir -p "$PREFIX/tmp"
...
echo -e "\e[36m[*] Mounting device filesystems...\e[0m"
sleep 0.4
ARGS="--kill-on-exit"
...
if [ "$FAILSAFE" = true ] && [ "$INSTALLING" != true ]; then
    ...
    echo -e "\e[36m[*] Starting Alpine Linux container...\e[0m"
    sleep 0.4
